...
void CuDNNSigmoidLayer<Dtype>::LayerSetUp(const vector<Tensor<Dtype>*>& bottom,
	...
	cudnn::CreateActivationDescriptor<Dtype>(&activation_desc_, CUDNN_ACTIVATION_SIGMOID); //mode是Sigmoid
	handles_setup_ = true;
}

//重写 reshape接口
template <typename Dtype>
void CuDNNSigmoidLayer<Dtype>::Reshape(const vector<Tensor<Dtype>*>& bottom,
                                       const vector<Tensor<Dtype>*>& top) {
